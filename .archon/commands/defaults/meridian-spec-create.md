...
```bash
cat .meridian/config/project.yaml 2>/dev/null || echo "MISSING"
```

If missing: stop and tell the user to run `setup` first.
If present: read `spec-source` setting. This command supports `spec-source: meridian` only.
...
- Relevant test files
- Related specs in `.meridian/specs/` (check for existing work)

Feature slug: derive from "$ARGUMENTS" (lowercase, hyphens, no spaces). Spec directory: `.meridian/specs/<feature-slug>/`

```bash
ls .meridian/specs/<feature-slug>/ 2>/dev/null || echo "NEW"
```
...
Create `.meridian/specs/<feature-slug>/requirements.md`:
...
Create `.meridian/specs/<feature-slug>/design.md`:
...
Create `.meridian/specs/<feature-slug>/tasks.md`:
...
- Spec location: `.meridian/specs/<feature-slug>/`
