...
argument-hint: <.meridian/specs/feature-name/> or <path/to/plan.md>
---
...
```bash
find . -name "tasks.md" -path "*/.meridian/specs/*" 2>/dev/null
```
...
- Spec location: .meridian/specs/<feature>/
...
Follow `meridian-implement.md` Phase 0–1 (detect package manager, read plan, extract tasks).
...
Present the list of modified files for the next `checkpoint` step.
