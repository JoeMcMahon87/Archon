...
Usage:
    python3 ~/.meridian/scripts/update-harness.py --platform claude-code
    python3 ~/.meridian/scripts/update-harness.py --platform opencode
    python3 ~/.meridian/scripts/update-harness.py --all
"""
...
MERIDIAN_HOME = Path.cwd() / ".meridian"
TEMPLATES_DIR = MERIDIAN_HOME / "harness-templates"
...
def scan_steering_files() -> list[str]:
    """Scan ~/.meridian/steering/ for files that should be in rules/."""
    steering_dir = MERIDIAN_HOME / "steering"
...
def scan_scripts() -> list[dict]:
    """Scan ~/.meridian/scripts/ for validation scripts."""
    scripts_dir = MERIDIAN_HOME / "scripts"
...
def scan_skills() -> list[str]:
    """Scan ~/.meridian/skills/ for available skills."""
    cache_file = MERIDIAN_HOME / "skills" / ".skills-cache.txt"
    if cache_file.exists():
        return [line.strip() for line in cache_file.read_text().splitlines() if line.strip()]

    skills_dir = MERIDIAN_HOME / "skills"
...
def generate_scripts_table(scripts: list[dict]) -> str:
    ...
    for s in scripts:
        cmd = f"`python3 ~/.meridian/scripts/{s['name']} <args>`"
...
def generate_skills_block(skills: list[str]) -> str:
    ...
    if not skills:
        return "No skills detected. Run `python3 ~/.meridian/skills/update-agent.py --force` to refresh."

    lines = ["Available skills in `~/.meridian/skills/` (loaded via symlink at `~/.claude/skills/`):", ""]
    for skill in skills:
    ...
    lines.extend([
        "",
        "**Loading a skill:**",
        "1. Read `~/.meridian/skills/<skill-name>/SKILL.md` for context",
        ...
        "- ALWAYS use absolute paths: `python3 ~/.meridian/skills/<skill-name>/scripts/<script>.py`",
...
def generate_knowledge_block() -> str:
    ...
    return (
        ...
        "| Existing specs | `.meridian/specs/` |\n"
        "| Shared components | `.meridian/registry/` |\n"
        "| Architecture decisions | `docs/` |\n"
        "\n"
        "**When NOT to search**: exact-path reads, files already in context, steering rules, active edits.\n"
        "\n"
        "**Discovery**: Run `python3 ~/.meridian/scripts/knowledge-init.py --project-dir $(pwd) --dry-run`\n"
