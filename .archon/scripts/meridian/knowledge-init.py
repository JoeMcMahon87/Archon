...
Usage:
    python3 ~/.meridian/scripts/knowledge-init.py --project-dir /path/to/project [--force] [--dry-run]
    python3 ~/.meridian/scripts/knowledge-init.py --configure-only
"""
...
_KB_CANDIDATES = [
    ...
    {
        "subdir": ".meridian/registry",
        "index_type": "Fast",
        "include": ["**/*.yaml", "**/*.yml"],
        "description": "Shared component registry",
    },
    {
        "subdir": ".meridian/config",
        "index_type": "Fast",
        "include": ["**/*.yaml", "**/*.yml", "**/*.md"],
...
FRESHNESS_FILE = Path.cwd() / ".meridian" / ".knowledge-freshness.json"
...
def get_project_root() -> Path | None:
    ...
    for i, arg in enumerate(sys.argv):
        if arg == "--project-dir" and i + 1 < len(sys.argv):
            ...
            if (candidate / ".meridian" / "config" / "project.yaml").exists():
                return candidate

    cwd = Path.cwd()
    if (cwd / ".meridian" / "config" / "project.yaml").exists():
        return cwd
    return None
...
def configure_settings(dry_run: bool = False) -> None:
    """Apply meridian-cli knowledge settings."""
    print("Configuring meridian-cli knowledge settings...")
    for key, value in KIRO_SETTINGS.items():
        if dry_run:
            print(f"  [dry-run] meridian-cli settings {key} {value}")
        else:
            result = run(["meridian-cli", "settings", key, value])
...
def main() -> None:
    ...
    if not project_root:
        ...
        print("  Expected: .meridian/config/project.yaml in --project-dir or cwd")
        return
