...
Loads variant definition from ~/.meridian/config/ssdf-pipeline-variants/ and provides
...
def load_pipeline_variant(project_dir: Path | None = None) -> PipelineVariant | None:
    ...
    yaml_path = root / ".meridian" / "config" / "project.yaml"
    ...
    variants_dir = os.environ.get("SSDF_VARIANTS_DIR",
                                   str(Path.cwd() / ".meridian" / "config" / "ssdf-pipeline-variants"))
