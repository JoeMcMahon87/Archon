...
Usage:
    python3 ~/.meridian/scripts/validate-spec.py .meridian/specs/<spec-name>   # Validate one spec
    python3 ~/.meridian/scripts/validate-spec.py --all                      # Validate all specs
    python3 ~/.meridian/scripts/validate-spec.py --verify .meridian/specs/<spec-name>  # Validate + verify implementation
    python3 ~/.meridian/scripts/validate-spec.py                            # Interactive menu (terminal)
                                                                # or list specs (AI/non-TTY)

Modes:
    Direct path   — Validates the specified spec directory. Use from AI or terminal.
    --all         — Validates every spec under .meridian/specs/ sequentially.
...
try:
    _scripts_dir = str(Path.cwd() / ".meridian" / "scripts")
...
REMEDIATION = {
    ...
    "No test-config": "→ Add test-config section to .meridian/config/project.yaml",
...
def find_project_root(spec_path: Path) -> Path | None:
    """Walk up from spec path to find project root (contains .meridian/)."""
    current = spec_path.resolve()
    while current != current.parent:
        if (current / ".meridian").is_dir():
            return current
...
def load_testing_config(project_root: Path) -> dict:
    ...
    config_path = project_root / ".meridian" / "config" / "project.yaml"
...
def resolve_design_md(project_root: Path, config: dict) -> Path | None:
    """Resolve DESIGN.md using priority chain: config path > local .meridian/config/ > global ~/.meridian/config/."""
    # Priority 1: explicit path in project.yaml
    if config.get("path"):
        candidate = project_root / config["path"]
        if candidate.exists():
            return candidate
    # Priority 2: local .meridian/config/
    local = project_root / ".meridian" / "config" / "DESIGN.md"
    if local.exists():
        return local
    # Priority 3: global ~/.meridian/config/
    global_path = Path.cwd() / ".meridian" / "config" / "DESIGN.md"
...
def check_design_system(design_text: str, project_root: Path) -> list[tuple[str, str, str]]:
    ...
    if resolved:
        ...
        results.append((WARN, "Design system", "design.md references frontend/UI but no DESIGN.md found (checked: config path, .meridian/config/DESIGN.md, ~/.meridian/config/DESIGN.md)"))
    return results
...
def validate_spec(spec_dir: str, json_mode: bool = False) -> tuple[int, list[tuple[str, str, str]], int]:
    ...
    if project_root:
        ...
        results.append((WARN, "Testing", "Could not find project root (.meridian/) — skipping test validation"))
    ...
    if (
        ...
        catalog_path = Path.cwd() / ".meridian" / "steering" / "compliance-pattern-catalog.md"
        if catalog_path.exists():
            all_specs_dir = str(project_root / ".meridian" / "specs")
            try:
...
def find_specs(project_root: Path) -> list[Path]:
    """Find all spec directories under .meridian/specs/."""
    specs_dir = project_root / ".meridian" / "specs"
...
def interactive_select(project_root: Path) -> Path | None:
    ...
    if not specs:
        print("No specs found in .meridian/specs/")
        return None

    specs_base = project_root / ".meridian" / "specs"
    ...
    try:
        ...
        print("  python3 ~/.meridian/scripts/validate-spec.py .meridian/specs/<spec-name>")
        print("  python3 ~/.meridian/scripts/validate-spec.py --all")
        return None
...
def main() -> None:
    project_root = find_project_root(Path.cwd() / ".meridian" / "specs")
    if not project_root:
    ...
    if all_mode:
        specs = find_specs(project_root)
        if not specs:
            print("No specs found in .meridian/specs/", file=sys.stderr)
            sys.exit(2)
        all_stats: list[dict] = []
        any_failure = False
        for spec in specs:
