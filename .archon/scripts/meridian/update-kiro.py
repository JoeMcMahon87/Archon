#!/usr/bin/env python3
"""Detect installed Kiro IDE and meridian-cli versions, check for updates, and upgrade when behind."""
...
def get_installed_cli_version():
    out, rc = run("meridian-cli --version 2>/dev/null")
    if rc == 0 and out:
        # First line: "meridian-cli X.Y.Z"
        for line in out.splitlines():
            if line.startswith("meridian-cli"):
                return line.split()[-1]
...
def upgrade_cli(index, latest, arch):
    ...
    if not pkg:
        print(f"  No .deb package found for {arch}. Try: meridian-cli update -y")
        return False

    download_url = f"{CLI_INDEX_URL.rsplit('/', 1)[0]}/{pkg['download']}"
    tmp = f"/tmp/meridian-cli-{latest}.deb"
    expected_sha = pkg.get("sha256", "")

    print(f"  Downloading meridian-cli {latest} ...")
...
def upgrade_ide():
    """Attempt IDE upgrade. Since the update API is restricted, use meridian-cli update or direct .deb."""
    print("  Attempting IDE update via meridian-cli update ...")
    _, rc = run("meridian-cli update -y", capture=False)
    return rc == 0
...
def main():
    ...
    print(f"meridian-cli installed: {cli_installed or 'not found'}")
    ...
    try:
        ...
        print(f"meridian-cli latest:    {cli_latest or 'unknown'}")
    except Exception as e:
        print(f"meridian-cli latest:    failed to fetch ({e})")
    ...
    if cli_installed and cli_latest:
        if version_tuple(cli_installed) < version_tuple(cli_latest):
            ...
            print(f"meridian-cli status:    UPDATE AVAILABLE ({cli_installed} -> {cli_latest})")
        elif force:
            cli_needs_update = True
            print(f"meridian-cli status:    FORCE reinstall ({cli_installed})")
        else:
            print(f"meridian-cli status:    up to date")
    ...
    if ide_installed and ide_latest:
        ...
        print(f"Kiro IDE status:    cannot check (try: meridian-cli update -y)")
    ...
    if cli_needs_update and cli_index:
        print("-" * 40)
        print(f"Upgrading meridian-cli to {cli_latest} ...")
        if upgrade_cli(cli_index, cli_latest, arch):
            new_ver = get_installed_cli_version()
            print(f"  meridian-cli upgraded to {new_ver}")
        else:
            print("  meridian-cli upgrade failed")
            exit_code = 1
