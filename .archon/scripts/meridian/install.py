...
Usage:
    python3 ~/.meridian/scripts/install.py            # First-time install
    python3 ~/.meridian/scripts/install.py --upgrade   # Upgrade existing install
"""
...
def install(repo_root, upgrade=False):
    ...
    version_file = MERIDIAN_HOME / ".meridian-version"
    ...
    print(f"{action} GDIT-SDAF Framework v{version} → {MERIDIAN_HOME}\n")
    ...
    update_script = MERIDIAN_HOME / "skills" / "update-agent.py"
    ...
    print(f"  Location: {MERIDIAN_HOME}")
...
def main():
    ...
    if not (repo_root / "VERSION").exists():
        print("ERROR: VERSION file not found. Run from the Meridian repo root.")
        sys.exit(1)
