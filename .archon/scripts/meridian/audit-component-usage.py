...
Three-layer architecture:
  1. Registry (.meridian/registry/shared-registry.yaml) — pattern violations
  2. SysML (.meridian/specs/*/model.sysml) — contract violations (use relationships)
...
def load_registry(project_root):
    config_path = os.path.join(project_root, '.meridian', 'config', 'project.yaml')
...
def parse_sysml_uses(project_root):
    ...
    uses = []
    for sysml_path in glob.glob(os.path.join(project_root, '.meridian', 'specs', '*', 'model.sysml')):
        try:
