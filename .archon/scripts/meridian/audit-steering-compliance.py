...
Usage:
    python3 ~/.meridian/scripts/audit-steering-compliance.py .meridian/specs/<feature>/
"""
...
VALIDATE_SPEC = Path.home() / ".meridian/scripts/validate-spec.py"
...
def _read_project_yaml(project_dir: Path) -> dict:
    ...
    yaml_path = project_dir / ".meridian" / "config" / "project.yaml"
...
AGENT_CONFIG = Path.home() / ".meridian/agents/meridian.json"
...
def check_graphql_schema(project_dir: Path, cfg: dict) -> tuple[str, str]:
    """Run ~/.meridian/scripts/validate-graphql-schema.py if graphql-schema-validation is enabled.
    ...
    """
    script = Path.cwd() / ".meridian" / "scripts" / "validate-graphql-schema.py"
...
def check_design_system(project_dir: Path, spec_dir: Path) -> tuple[str, str]:
    ...
    paths_checked = "config path, .meridian/config/DESIGN.md, ~/.meridian/config/DESIGN.md"
...
def check_ssdf_evidence(project_dir: Path) -> tuple[str, str]:
    ...
    script = Path.cwd() / ".meridian" / "skills" / "ssdf-development" / "scripts" / "collect_evidence.py"
    if not script.exists():
