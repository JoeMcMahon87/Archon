...
try:
    _shared_dir = str(Path.cwd() / ".meridian" / "skills" / "shared")
...
DEFAULT_SCANNER_FILE_MAP: dict[str, list[str]] = {
    ...
    "pyright": [".py"],
    "py_compile": [".py"],
    "semgrep": [".py", ".js", ".ts", ".jsx", ".tsx", ".java", ".go", ".rb", ".rs"],
    "gitleaks": ["*"],
...
def check_graphql_schema(project_dir: Path, cfg: dict) -> tuple[str, str]:
    ...
    if result.returncode == 0:
        summary = next((l.strip() for l in output.splitlines() if "checked" in l.lower()), "passed")
        return "PASS", summary
